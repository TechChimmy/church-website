...
// ─── Section Groups (mirrors sidebar) ────────────────────────────────────────
const SECTION_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { href: "/admin/events#we-stay-active", translationKey: "weStayActiveMenu", icon: "🏃", descKey: "weStayActiveDesc" },
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-3 mb-10">
        {stats.map(s => (
          <Link
            ...
            href={s.href}
            prefetch={false}
            className="rounded-[8px] px-4 py-4 no-underline block transition-all hover:scale-[1.02]"
            style={{
              backgroundColor: s.urgent ? "var(--burgundy)" : "white",
              border: s.urgent ? "none" : "1px solid rgba(140,58,99,0.1)",
              boxShadow: s.urgent ? "0 4px 20px rgba(140,58,99,0.25)" : "0 2px 8px rgba(140,58,99,0.04)",
            }}
          >
            <p className="font-lato text-[9px] font-bold uppercase tracking-widest mb-1.5"
              style={{ color: s.urgent ? "rgba(255,255,255,0.65)" : "#9B8A90" }}>
              {s.label}
            </p>
            <p className="font-playfair text-[28px] font-bold leading-none"
        ...
        {SECTION_GROUPS.map(group => (
          <div key={group.label}>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <h2 className="font-lato text-[11px] font-bold uppercase tracking-[2px]"
                style={{ color: "#9B8A90" }}>
                {group.label}
              </h2>
              <div className="h-px flex-1" style={{ backgroundColor: "rgba(140,58,99,0.08)" }} />
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
              {group.items.map(c => (
                <Link
                  ...
                  prefetch={false}
                  className="bg-white rounded-[8px] px-5 py-4 no-underline block
                             transition-all hover:shadow-md hover:scale-[1.01] group"
                  style={{ border: "1px solid rgba(140,58,99,0.08)" }}
                >
                  <div className="flex items-center gap-3">
                    <div
                      className="w-9 h-9 rounded-[8px] flex items-center justify-center shrink-0 text-[18px]"
                      style={{ backgroundColor: "rgba(140,58,99,0.07)" }}
                    >
                      {c.icon}
                    </div>
                    <div className="min-w-0 flex-1">
                      <p className="font-lato font-bold text-[13px] leading-tight
                    ...
                    <svg viewBox="0 0 24 24" className="w-4 h-4 shrink-0 opacity-0 group-hover:opacity-100 transition-opacity"
