...
type AnswerItem = { id: string | number; imageLeft: boolean; title: string; body: string };

const ALL_ANSWERS: AnswerItem[] = [
...
export default function AnswersFromTheWord({ initialAnswers = [] }: { initialAnswers?: any[] }) {
  const [answers] = useState<AnswerItem[]>(() => {
    if (initialAnswers && initialAnswers.length > 0) {
      return initialAnswers.map((item, index) => ({
        id: item._id,
        imageLeft: index % 2 !== 0,
        title: item.question ?? "",
        body: item.answer ?? "",
  ...
  // Helper to map index to image ID
  const getImageSrc = (id: string | number, index: number) => {
    if (typeof id === 'number') {
      return `/images/answers/answer-${id}.jpg`;
    }
    // For Sanity-fetched items, assign a dynamic index from the query
    const staticId = (index % 7) + 1;
    return `/images/answers/answer-${staticId}.jpg`;
  };
          ...
          >
            {pageItems.map((item, idx) => (
              <div key={item.id}
                ...
                {/* Image */}
                <div className={(item.imageLeft ? "order-1" : "order-2") + " relative h-[210px] w-full"}>
                  <Image
                    src={getImageSrc(item.id, (page - 1) * PAGE_SIZE + idx)}
                    alt={item.title}
                    fill
